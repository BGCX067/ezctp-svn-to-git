 ...
 * Module Name  : ezctpStoreMarketData.c
 *
...
static void *store_thread_routine(void *arg)
{
	struct CThostFtdcDepthMarketDataField DepthMarketData, *pDepthMarketData;
	FILE *raw_fp = NULL;
	...
	run_state = 1;
	raw_fp = fopen(ezctp_RawFilePath, "r+b");
	if (raw_fp == NULL) {
		//printf("%s(%d) can't open [%s]\n", __func__, __LINE__, ezctp_RawFilePath);
	}

	pDepthMarketData = &DepthMarketData;

	/* do the save task */
	while (run_state == 1) {
		if (raw_fp != NULL) {
			while (!feof(raw_fp)) {
				/* first read from raw file */
				if (fread(pDepthMarketData, sizeof(struct CThostFtdcDepthMarketDataField), 1, raw_fp) == 1) {
					/* then store to SQLite DB */
...
int main(int argc, char **argv)
{
	...
	int threads_max = 0;
	int s;
	pthread_t sig_thread, store_thread;
	sigset_t sigset;
	...
	s = pthread_create(&store_thread, NULL, &store_thread_routine, (void *) &sigset);
	if (s != 0) {
		DBG("<6>ezctpSaveMarketData: pthread_create save\n");
		handle_error_en(s, "pthread_create store");
	}
