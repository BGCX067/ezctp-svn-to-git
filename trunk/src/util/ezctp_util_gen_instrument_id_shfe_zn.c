 ...
 * Module Name  : util/ezctp_util_gen_instrument_id_shfe_zn.c
 *
...
#define EZCTP_INSTRUMENT_ID_SHFE_ZN_STRING	"zn"
...
int ezctp_util_gen_instrument_id_shfe_zn(char *buf, size_t len, int idx)
{
	...
	if (idx > 0) {
		...
		count = snprintf(buf, len, "%s%d", EZCTP_INSTRUMENT_ID_SHFE_ZN_STRING, idx);
		if (count >= len) {
			return -1;
		}
	}
		...
		for(i = 0; i < 13; i++) {
			...
			ret = snprintf(buf+count, len-count, "%s%d", EZCTP_INSTRUMENT_ID_SHFE_ZN_STRING, idx);
			if (ret < 0) {
				return -1;
			}
			count += ret;
