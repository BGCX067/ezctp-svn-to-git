 ...
 * Module Name  : util/ezctp_util_gen_instrument_id_dce_y.c
 *
...
#define EZCTP_INSTRUMENT_ID_DCE_Y_STRING	"y"
...
int ezctp_util_gen_instrument_id_dce_y(char *buf, size_t len, int idx)
{
	...
	if (idx > 0) {
		...
		count = snprintf(buf, len, "%s%d", EZCTP_INSTRUMENT_ID_DCE_Y_STRING, idx);
		if (count >= len) {
			return -1;
		}
	}
		...
		for(i = 0; i < 14; i++) {
			...
			ret = snprintf(buf+count, len-count, "%s%d", EZCTP_INSTRUMENT_ID_DCE_Y_STRING, idx);
			if (ret < 0) {
				return -1;
			}
			count += ret;
